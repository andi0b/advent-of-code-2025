...
let vecToArray (vec: Vector<'a>) =
    let array = Array.zeroCreate Vector<'a>.Count
    vec.CopyTo(array)
    array

let getPattern (vec: Vector<uint16>) =
    let vec' = vec.AsVector256()
    let lsb = Avx2.And(vec', Vector256<uint16>.One)
    let sign = Avx2.ShiftLeftLogical(lsb, 15uy)
...
let part2 =
    let minButtonPresses machine =
        let patterns =
            machine.buttons
            |> Seq.map (fun b ->
                let array = Array.zeroCreate machine.jolts.Length

                for i in b do
                    array[i] <- 1us

                array |> toRightAlignedVector)
            |> Seq.toList
            |> allCombinations
            |> Seq.map (fun buttons ->
                    let sum = buttons |> List.reduce (+)
        ...
        let rec solveStep target =
            if target = Vector<uint16>.Zero then
                ...
                | ValueSome pattern ->
                    pattern
                    |> Array.choose (fun (sum, count) ->
                        if Vector.LessThanAny(target, sum) then
                            None
                        else
                            let newTarget = (target - sum) / Vector<uint16>(2us)
                            solveStep newTarget |> Option.map (fun x -> x * 2 + count))
                    |> function
                        | [||] -> None
                        | array -> Some(Array.min array)
                | ValueNone -> None

        let target = machine.jolts |> Array.map uint16 |> toRightAlignedVector
        solveStep target |> Option.defaultValue 100000
